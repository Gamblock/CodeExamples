...
namespace PixelCrushers.DialogueSystem.Articy
{
    ...
    public class ArticyConverterWindow : EditorWindow
    {
        ...
        /// Draws the Overwrite checkbox.
        /// </summary>
        private void DrawCopyLocalFieldsCheckbox()
        {
            EditorGUI.BeginChangeCheck();
            prefs.CopyLocalFields = EditorGUILayout.Toggle("Copy Local Fields", prefs.CopyLocalFields);
            if (EditorGUI.EndChangeCheck()) ConverterPrefsTools.Save(prefs);
        }
            ...
            if (articyData != null) try
                {
                    ...
                    DialogueDatabase localDatabaseCopy = DatabaseUtility.CreateDialogueDatabaseInstance();
                    DialogueDatabase database = LoadOrCreateDatabase(assetName, localDatabaseCopy);
                    if (database == null)
                    {
                    ...
                    else
                    {
                        ...
                        if (prefs.CopyLocalFields)
                        {
                            DatabaseImportUtility.CopyFieldsInConversations(localDatabaseCopy, 
                                database, "Articy Id");
                            DatabaseImportUtility.CopyCallbacksInConversations(
                                localDatabaseCopy,
                                database,
                                "fields",
                                "title",
        ...
        /// Asset filename.
        /// </param>
        private DialogueDatabase LoadOrCreateDatabase(string filename, DialogueDatabase localDatabaseCopy)
        {
            var assetPath = prefs.OutputFolder;
            if (!assetPath.EndsWith("/")) assetPath += "/";
            assetPath += filename + ".asset";
            DialogueDatabase database = null;
            if (prefs.CopyLocalFields || prefs.Overwrite)
            {
            ...
            if (prefs.CopyLocalFields)
            {
                if (database != null) localDatabaseCopy.Add(database);
            }
